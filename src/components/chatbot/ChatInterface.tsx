...
import { Send, User, Sparkles, Loader2, Bot, MessageCircle } from 'lucide-react';
...
export function ChatInterface() {
  ...
  const handleSubmit = async (e?: FormEvent<HTMLFormElement>) => {
    ...
    if (!user) {
        toast({
            ...
            description: 'Please log in to start a conversation with the AI assistant.',
        });
        return;
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-10rem)] w-full bg-background border rounded-xl shadow-md mt-[3px]">
      <header className="flex-shrink-0 flex items-center p-3 border-b border-border/40 bg-background/95 backdrop-blur-sm z-10 rounded-t-xl">
        ...
        <div className="flex flex-col">
            ...
            <p className={cn("text-xs", isLoggedIn ? "text-muted-foreground" : "text-destructive font-medium")}>
      ...
      <footer className="flex-shrink-0 p-3 sm:p-4 border-t border-border/40 bg-background/95 z-10 rounded-b-xl">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
            <Textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder={authLoading ? "Authenticating..." : (!isLoggedIn ? "Please log in to chat with CyberMozhi." : "Ask anything...")}
            ...
            />
            <Button type="submit" size="icon" className="h-10 w-10 shadow-sm rounded-full flex-shrink-0" disabled={isSendingMessage || authLoading || !isLoggedIn || !input.trim()} aria-label="Send message">
